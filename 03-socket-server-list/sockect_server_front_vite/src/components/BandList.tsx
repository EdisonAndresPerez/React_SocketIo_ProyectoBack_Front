...
const BandList: React.FC<BandListProps> = ({ bands, onVote, onDelete }) => {
  if (bands.length === 0) {
    return (
          ...
          {sortedBands.map((band, index) => (
            <tr key={band.id} className="align-middle">
              <td className="fw-bold text-primary">
                {index === 0 && <i className="bi bi-trophy-fill text-warning me-1"></i>}#{index + 1}
              </td>
              <td>
                <div className="fw-semibold text-dark">{band.name}</div>
              </td>
              <td className="text-center">
                <span className="badge bg-primary rounded-pill fs-6 px-3 py-2">{band.votes}</span>
              </td>
              <td className="text-center">
